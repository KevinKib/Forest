  ...
  begin
    ...
    if civ.CurrentRecherche = 'Aucun' then
      begin
        ...
        if      (recherche = 'Epée Tranchante')     then lancerRecherche := not(civ.recherche_EpeeTranchante)
        ...
        else if (recherche = 'Bac à Compost')       then lancerRecherche := not(civ.recherche_BacCompost)
        else if (recherche = 'Brocante Forestière') then lancerRecherche := not(civ.recherche_Brocante)
        else if (recherche = 'Stratégie')           then lancerRecherche := not(civ.recherche_Strategie);
    ...
    else texteInfoJoueur := 'Vous avez déjà une recherche en cours !';
  ...
  * menu descriptif de chaque recherche, plus détaillé}
  ...
  begin
    ...
    //Affichage ameliorations
      ...
      if not(civ.recherche_EpeeTranchante)  then ecrireEnPositionXY(6,15,'1 - Epée Tranchante');
      ...
      if not(civ.recherche_BacCompost)      then ecrireEnPositionXY(6,18,'4 - Bac à compost');
      if not(civ.recherche_Brocante)        then ecrireEnPositionXY(6,19,'5 - La brocante forestière');
      if not(civ.recherche_Strategie)       then ecrireEnPositionXY(6,20,'6 - Stratégie de Combat Contre Les Humains');
