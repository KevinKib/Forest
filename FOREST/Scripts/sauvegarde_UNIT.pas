...
{Cette unité a pour rôle la gestion des fichiers de sauvegarde binaires.}
...
uses
	...
	{RÔLE : Permet d'écrire les données du record de sauvegarde dans un fichier binaire au nom du joueur}
	procedure readFile(nbFile: Integer; var game: TGame; var civ: TCivilisation; var combat: TCombat);
	{RÔLE : Permet de récupérer les données du record de sauvegarde dans un fichier binaire au nom du joueur}
	procedure writeInfo(info: TInfo);
	{RÔLE : Permet d'écrire les noms de fichier dans informations.bin.}
	procedure readInfo(var info: TInfo);
	{RÔLE : Permet de récupérer les noms de fichier du fichier informations.bin.}
...
{RÔLE :  Assembler les trois records du jeu en un seul record afin de n'utiliser qu'un seul fichier pour la sauvegarde}
...
{RÔLE : Extrait le record utilisé pour la sauvegarde pour le rendre utilisable dans le jeu}
...
* (qui stocke les noms de fichier), on enregistre ensuite le nom du fichier de sauvegarde dans la variable qui stocke le nom du héros, on ecrit le nom
...
begin
  // On enregistre les données de nos trois variables record dans le record de sauvegarde.
  ...
  // On enregistre le nom du fichier de sauvegarde dans la variable qui stocke le nom du héros.
  case nbFile of
...
* dans la variable passee en paramètre, on ferme le fichier puis on entregistre les donnees de notre variable save dans les trois variables record}
begin

  // On assigne le fichier sélectionné par le joueur à notre programme.
  assign(saveFile,SAVE_FILE_NAMES[nbFile]);

  // On ouvre notre fichier en lecture.
  reset(saveFile);

  // On enregistre les données du fichier dans la variable passée en paramètre.
  read(saveFile, save);

  // On ferme notre fichier.
  close(saveFile);

  // On enregistre les données de notre variable save dans nos trois variables record.
...
procedure readInfo(var info: TInfo);
{PRINCIPE : on assigne un fichier a notre programme, on l'ouvre en lecture, on enregistre les données du fichier dans la variable passee en parametre et on le ferme}
begin
