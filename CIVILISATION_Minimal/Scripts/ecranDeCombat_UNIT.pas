...
{RÔLE : Cette procedure a pour role d'afficher l'ecran de combat en proposant l'attaque de différents camps ennemis}
...
implementation
  ...
  {PRINCIPE : On crée les variables des forces ennemis dans les petits/grands camps grace aux valeurs maximum et minimum que peuvent avoir les forces ennemies et les forces du joueur}
  begin
  ...
  {RÔLE : Cette procédure a pour rôle d'initialiser les variables necessaires pour le combat}
  {PRINCIPE : On génère les valeurs recherchées grâce à la fonction random() fournie par Pascal dans l'intervalle permise (max min) }
  begin
  ...
  {RÔLE : Cette procédure a pour rôle de gérer l'attaque}
  {PRINCIPE : Selon l'argument passé en paramètre, on génère une perte de forces armées chez le joueur et l'ennemie
